...
ReaderID int,
DocID int references Copy(DocID),
CopyNo int references Copy(CopyNo),
LibID int references Copy(LibID),
...
DROP TRIGGER IF EXISTS `RetDoctoHistory`;
CREATE TRIGGER RetDoctoHistory
AFTER DELETE ON `Borrows`
FOR EACH ROW
INSERT INTO `cityLib`.`Borrow_History` 
(`ReaderID`, `DocID`,`CopyNo`,`DateCheckedOut`,`DateReturned`,`DaysLate`,`Fine`) 
VALUES 
